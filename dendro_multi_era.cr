...
struct Dendro
  ...
  struct Core
    ...
    @[JSON::Field(ignore: true)]
    getter layers = Array(Array(Point)).new
    @[JSON::Field(ignore: true)]
    getter sectors = Array(Float64?).new(SECTOR_NUM, nil)

    def initialize(@cp, @max_r, @life, @age = 1, @color = nil)
    end
  ...
  module Color
    ...
    def hex2rgb(h) : ColorValue
      c = h[1..6].hexbytes
      {c[0].to_i, c[1].to_i, c[2].to_i}
    end

    def rgb2hex(r, g, b) : String
      '#' + Bytes[r, g, b].hexstring
    end

    def step(f, t, p) : ColorValue
      return f if p <= 0
      return t if p >= 1
      {(f[0] + (t[0] - f[0]) * p).to_i,
       (f[1] + (t[1] - f[1]) * p).to_i,
  ...
  module Svg
    ...
    def build(color, cores, prng, layer_flow)
      XML.build_fragment(indent: %(  )) do |svg|
        ...
        svg.element(%(svg),
          ...
          version: %(1.1), fill: FILL, stroke: COLORS[color],
    ...
    private def build_core(core, color, prng, svg)
      ...
      core.layers.reverse.each.with_index do |points, i|
        ...
        svg.element(%(path), d: Path.from_points(points),
          stroke: stroke_color(core, color, i),
          %(fill-opacity): fo,
          %(stroke-width): sw, %(stroke-opacity): so)
      end
    end

    private def stroke_color(core, color, i)
      return "" unless c = core.color
      c1 = Color.hex2rgb(COLORS[c])
      c2 = Color.hex2rgb(COLORS[color])
      pos = -4 + 5 / core.layers.size * i
      Color.rgb2hex(*Color.step(c2, c1, pos))
    end
  end
