...
struct Dendro
  ...
  LAYER_SPACING  = {0.9, 0.7}   # start/end layer spacing
    ...
    private def build_core(core, color, prng, svg)
      core.layers.reverse.each.with_index do |points, i|
        ...
        svg.element(%(path), d: Path.from_points(points),
          %(fill-opacity): step_opacity(core, i),
          %(stroke-width): sw, %(stroke-opacity): step_opacity(core, i))
      end
    end

    private def step_opacity(core, i)
      pos = -3 + 6 / core.layers.size * i
      Blend.step(0, 1, pos)
    end
  end
